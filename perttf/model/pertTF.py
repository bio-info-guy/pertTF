...
class PerturbationTFModel(BaseModel):
    def __init__(self, n_pert: int, nlayers_pert: int, n_ps: int, *args, **kwargs):
        self.pred_lochness_next = kwargs.pop(
            "pred_lochness_next", False
        )  # additional optional parameter to ask whether to predict lochness scores
        ps_decoder2_nlayer = kwargs.pop(
            "ps_decoder2_nlayer", 3
        )  # additional parameter to specify ps_decoder2 nlayer
        self.pert_pad_id = kwargs.pop("pert_pad_id", None)  # get the pert_pad_id
        self.pert_dim = kwargs.pop("pert_dim", None)
        self.flow_matching = kwargs.pop("flow_matching", False)
        self.flow_matching_active = kwargs.pop(
            "flow_matching_active", self.flow_matching
        )
        self.flow_reference_mode = kwargs.pop("flow_reference_mode", "source")
        self.flow_state_mode = kwargs.pop("flow_state_mode", "residual")
        self.flow_noise_mode = kwargs.pop("flow_noise_mode", "gaussian")
        self.flow_noise_scale = kwargs.pop("flow_noise_scale", 1.0)
        ...
        self.flow_training_rollout_policy = kwargs.pop(
        ...
        self.pert_encoder = PertLabelEncoder(
            n_pert, pert_dim, padding_idx=self.pert_pad_id
        )
        self.pert_exp_encoder = PertExpEncoder(d_model, self.pert_dim)
        self.flow_perturbation_generator = None
        if self.flow_matching:
            self.flow_perturbation_generator = FlowPerturbationGenerator(
                d_model=d_model,
                pert_dim=self.pert_dim,
                reference_mode=self.flow_reference_mode,
                state_mode=self.flow_state_mode,
                noise_mode=self.flow_noise_mode,
                noise_scale=self.flow_noise_scale,
        ...
        if self.use_fast_transformer:
            ...
            if self.fast_transformer_backend == "sdpa":
                ...
                try:
                    ...
                    print(ee)
                    print(
                        "pytorch sdpa attention setup failed, falling back to native pytorch attention"
                    )
            ...
            if encoder_layers is None:
                print(
                    f"fast_transformer_backend={self.fast_transformer_backend} — using vanilla TransformerEncoderLayer"
                )
                self.use_fast_transformer = False
                self.fast_transformer_backend = "vanilla"
                encoder_layers = TransformerEncoderLayer(
        ...
        if self.pred_lochness_next:
            self.ps_decoder2 = PSDecoder(
                d_model, 1, nlayers=ps_decoder2_nlayer, geneinput=True
            )
        else:
    ...
    def _generate_next_cell_embedding(
        self,
        ...
        if self._flow_matching_is_active():
            ...
            if return_flow_dict:
                flow_dict = flow_output
                cell_emb_next = flow_output["latent_final"]
            else:
                cell_emb_next = flow_output
        else:
            cell_emb_next = self.pert_exp_encoder(tf_concat)
        if return_flow_dict:
            return cell_emb_next, tf_concat, flow_dict
        return cell_emb_next, tf_concat

    def _flow_matching_is_active(self) -> bool:
        return bool(
    ...
    def _encode_detached_target_latent(
        ...
        prev_gene_token_embs = getattr(self, "cur_gene_token_embs", None)
        ...
        with torch.no_grad():
            ...
            target_latent = self._get_cell_emb_from_layer(transformer_output, values)
        if was_training:
            self.train()
        self.cur_gene_token_embs = prev_gene_token_embs
        return target_latent.detach()

    def _build_flow_training_dict(
        self,
        source_latent: Tensor,
        pert_emb_next: Tensor,
        next_gene_ids: Optional[Tensor],
        next_values: Optional[Tensor],
        next_src_key_padding_mask: Optional[Tensor],
        batch_labels: Optional[Tensor] = None,
        target_pert_labels: Optional[Tensor] = None,
        batch_emb: Optional[Tensor] = None,
        sf_next: Optional[Tensor] = None,
    ) -> Optional[Dict[str, Tensor]]:
        if (
            not self._flow_matching_is_active()
            or self.flow_perturbation_generator is None
            or next_gene_ids is None
            or next_values is None
            or next_src_key_padding_mask is None
        ):
            return None

        target_latent = self._encode_detached_target_latent(
            next_gene_ids,
            next_values,
            next_src_key_padding_mask,
            batch_labels=batch_labels,
            pert_labels=target_pert_labels,
        )
        training_path = self.flow_perturbation_generator.sample_training_path(
            source_latent=source_latent,
            target_latent=target_latent,
            pert_emb=pert_emb_next,
        )
        pred_velocity = self.flow_perturbation_generator.predict_velocity(
            training_path["state_t"],
            training_path["t"],
            source_latent,
            pert_emb_next,
        )
        endpoint_dict = self.flow_perturbation_generator.reconstruct_endpoint(
            training_path["state_t"],
            pred_velocity,
            training_path["t"],
            training_path["reference_latent"],
        )
        flow_training_dict = {
            **training_path,
            "target_latent": target_latent,
        ...
        if hasattr(self, "mvc_decoder"):
            endpoint_mvc_output = self.mvc_decoder(
                endpoint_dict["latent_1_hat"]
                if not self.use_batch_labels
                else torch.cat([endpoint_dict["latent_1_hat"], batch_emb], dim=1),
                self.cur_gene_token_embs,
                target_size_factor=sf_next if self.distribution is not None else None,
            )
            flow_training_dict["endpoint_mvc_output"] = endpoint_mvc_output
        return flow_training_dict

    def _flow_training_uses_rollout(self) -> bool:
        latent_source = getattr(self, "flow_train_next_latent_source", "endpoint")
        rollout_policy = getattr(self, "flow_training_rollout_policy", "off")
        if latent_source not in {"endpoint", "rollout"}:
            raise ValueError(
                "flow_train_next_latent_source must be 'endpoint' or 'rollout', "
                f"got {latent_source!r}"
            )
        if rollout_policy not in {"off", "detach", "full"}:
            raise ValueError(
                "flow_training_rollout_policy must be 'off', 'detach', or 'full', "
                f"got {rollout_policy!r}"
            )
        return latent_source == "rollout" and rollout_policy in {"detach", "full"}

    def _select_flow_training_next_cell_embedding(
        self,
        flow_training_dict: Dict[str, Tensor],
        rollout_cell_emb_next: Optional[Tensor],
    ) -> Tensor:
        if not self._flow_training_uses_rollout():
            return flow_training_dict["latent_1_hat"]

        if rollout_cell_emb_next is None:
            raise RuntimeError(
                "flow rollout next latent requested for training, but no rollout latent was computed"
            )

        if self.flow_training_rollout_policy == "detach":
            return rollout_cell_emb_next.detach()
        return rollout_cell_emb_next
    ...
    def forward(
        ...
        next_gene_ids: Optional[Tensor] = None,
        next_values: Optional[Tensor] = None,
        next_src_key_padding_mask: Optional[Tensor] = None,
        ...
        if pert_labels_next is not None:  # and False:
            ...
            if self._flow_matching_is_active():
                flow_training_dict = self._build_flow_training_dict(
                    source_latent=cell_emb_orig,
                    ...
                    batch_labels=batch_labels,
                    target_pert_labels=pert_labels_next
                    if pert_labels is not None
                    else None,
            ...
            flow_rollout = None
            rollout_cell_emb_next = None
            if (
                not self._flow_matching_is_active()
                or flow_training_dict is None
                or self._flow_training_uses_rollout()
            ):
                rollout_cell_emb_next, tf_concat, flow_rollout = (
            ...
            if flow_training_dict is not None:
                cell_emb_next = self._select_flow_training_next_cell_embedding(
                    flow_training_dict,
                    rollout_cell_emb_next,
                )
            else:
                cell_emb_next = rollout_cell_emb_next

            output["contrastive_dict"]["next_emb0"] = cell_emb_next
            if flow_rollout is not None:
                output["flow_dict"] = {"inference_rollout": flow_rollout}
        else:
        ...
        if flow_training_dict is not None:
            if "flow_dict" not in output:
                output["flow_dict"] = {}
            output["flow_dict"]["training"] = flow_training_dict

        if CLS:
            output["cls_output"] = self.cls_decoder(cell_emb)  # (batch, n_cls)
            output["cls_output_next"] = self.cls_decoder(
                cell_emb_next
            )  # (batch, n_cls)

        cur_gene_token_embs = (
            self.encoder(mvc_src) if mvc_src is not None else self.cur_gene_token_embs
        )
        if MVC and hasattr(self, "mvc_decoder"):
            ...
            if (
                self.explicit_zero_prob and False
            ):  # bernoulli sampling is meaningless here
        ...
        if PERTPRED:
            ...
            output["pert_output_next"] = self.pert_decoder(
                cell_emb_next
            )  # (batch, n_cls)
        ...
        if PSPRED and self.ps_decoder is not None:
            ...
            if self.pred_lochness_next:
                ...
                output["ps_output_next"] = self.ps_decoder2(
                    tf_concat
                )  # this is the concatenation of cell embedding and predictive label (next)
            else:
                output["ps_output_next"] = self.ps_decoder(
                    cell_emb_next
                )  # (batch, n_cls)
        return output
    ...
    def encode_batch_with_perturb(
        ...
        shape_ps = (
            (N, self.n_ps) if time_step is not None else (N, src.size(1), self.n_ps)
        )
        ...
        if predict_expr:
            ...
            mvc_expr_shape = (
                (N, src.size(1)) if mvc_src is None else (N, mvc_src.size(1))
            )
            mlm_outputs, mlm_zero_outputs = (
                array_func(mlm_expr_shape, dtype=float32_),
                array_func(mlm_expr_shape, dtype=float32_),
            )
            mvc_outputs, mvc_zero_outputs = (
                array_func(mvc_expr_shape, dtype=float32_),
                array_func(mvc_expr_shape, dtype=float32_),
            )
            mvc_next_outputs, mvc_next_zero_outputs = (
                array_func(mvc_expr_shape, dtype=float32_),
                array_func(mvc_expr_shape, dtype=float32_),
            )
            if self.distribution in ["nb", "hnb", "zinb", "zig"]:
                mvc_param2_outputs, mvc_next_param2_outputs = (
                    array_func(mvc_expr_shape, dtype=float32_),
                    array_func(mvc_expr_shape, dtype=float32_),
                )
        for i in trange(0, N, batch_size):
            ...
            pert_labels_next_d = (
                pert_labels_next[i : i + batch_size].to(device)
                if pert_labels_next is not None
                else None
            )
            mvc_src_d = (
                mvc_src[i : i + batch_size].to(device) if mvc_src is not None else None
            )
            ...
            if pert_labels_next_d is not None:
                ...
                if output_to_cpu:
                    ...
                    cell_emb_next_cpu = cell_emb_next
                if return_np:
            ...
            if predict_expr:
                ...
                cur_gene_token_embs = (
                    self.encoder(mvc_src_d)
                    if mvc_src_d is not None
                    else self.cur_gene_token_embs
                )
                mvc_output = self.mvc_decoder(
                    cell_emb
                    if not self.use_batch_labels
                    else torch.cat(
                        [cell_emb, batch_emb], dim=1
                    ),  # else cell_emb + batch_emb,
                    cur_gene_token_embs,
                ...
                mlm_pred, mlm_zero_probs = (
                    mlm_output["pred"],
                    mlm_output["zero_probs"]
                    if self.explicit_zero_prob
                    else torch.ones_like(mlm_output["pred"]),
                )
                if return_np:
                    mlm_pred, mlm_zero_probs = (
                        mlm_pred.cpu().numpy(),
                        mlm_zero_probs.cpu().numpy(),
                    )
                mvc_generator = DistributionGenerator(self.distribution)
                mvc_output = mvc_generator.generate(
                    mvc_output,
                    sample=sample,
                    to_numpy=return_np,
                    device=next(self.parameters()).device,
                )
                mvc_output_next = mvc_generator.generate(
                    mvc_output_next,
                    sample=sample,
                    to_numpy=return_np,
                    device=next(self.parameters()).device,
                )
                mvc_pred, mvc_param2, mvc_zero_probs = (
                    mvc_output["pred"],
                    mvc_output["param2"],
                    mvc_output["zero_probs"],
                )
                mvc_pred_next, mvc_param2_next, mvc_zero_probs_next = (
                    mvc_output_next["pred"],
                    mvc_output_next["param2"],
                    mvc_output_next["zero_probs"],
                )
                (
                    mlm_outputs[i : i + batch_size],
                    mvc_outputs[i : i + batch_size],
                    mvc_next_outputs[i : i + batch_size],
                ) = mlm_pred, mvc_pred, mvc_pred_next
                if mvc_param2 is not None and mvc_param2_next is not None:
                    (
                        mvc_param2_outputs[i : i + batch_size],
                        mvc_next_param2_outputs[i : i + batch_size],
                    ) = mvc_param2, mvc_param2_next
                if self.explicit_zero_prob:
                    (
                        mlm_zero_outputs[i : i + batch_size],
                        mvc_zero_outputs[i : i + batch_size],
                        mvc_next_zero_outputs[i : i + batch_size],
                    ) = mlm_zero_probs, mvc_zero_probs, mvc_zero_probs_next

        if predict_expr:
            expr_dict["mlm_expr"], expr_dict["mvc_expr"], expr_dict["mvc_next_expr"] = (
                mlm_outputs[:, 1:],
                mvc_outputs[:, 1:],
                mvc_next_outputs[:, 1:],
            )
            if self.explicit_zero_prob:
                (
                    expr_dict["mlm_expr_zero"],
                    expr_dict["mvc_expr_zero"],
                    expr_dict["mvc_next_expr_zero"],
                ) = (
                    mlm_zero_outputs[:, 1:],
                    mvc_zero_outputs[:, 1:],
                    mvc_next_zero_outputs[:, 1:],
                )
            if self.distribution in ["nb", "hnb", "zinb", "zig"]:
                expr_dict["mvc_param2"], expr_dict["mvc_next_param2"] = (
                    mvc_param2_outputs[:, 1:],
                    mvc_next_param2_outputs[:, 1:],
                )
        return (
            outputs,
            outputs_next,
