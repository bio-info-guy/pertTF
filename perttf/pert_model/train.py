...
from perttf.pert_model.loss import compute_hard_negative_link_loss, compute_margin_link_loss
import torch
...
def extract_master_edge_index(rgcn_module, to_cpu=True):
    ...
    for i in range(rgcn_module.num_relations):
        ...
        adj_t = SparseTensor(
            rowptr=rowptr, 
            col=col, 
            sparse_sizes=(rgcn_module.num_nodes, rgcn_module.num_nodes),
            is_sorted=True
        )
...
def train(model: nn.Module,
    ...
    for batch, batch_data in enumerate(loader):
        ...
        with torch.cuda.amp.autocast(enabled=config.amp):
            ...
            if config.get('link_loss', None) is not None:
...
def wrapper_train(model, config, data_gen,
    ...
    if model.pert_encoder is not None:
